...
const FooterLink = ({ heading, items }) => {
  return (
    <div className="flex-1 justify-start items-start">
      <h1 className="font-poppins dark:text-white text-nft-black-1 font-semibold text-xl mb-10">{heading}</h1>
      {items.map((item, index) => (
        <p className="font-poppins dark:text-white text-nft-black-1 font-normal text-base cursor-pointer dark:hover:text-nft-gray-1 hover:text-nft-black-1 my-3">{item}</p>
      ))}
    </div>
  );
};
...
const Footer = () => {
  ...
  return (
    <footer className="flexCenter flex-col border-t dark:border-nft-black-1 border-nft-gray-1 sm:py-8 py-16 ">
      <div className="w-full minmd:w-4/5 flex flex-row md:flex-col sm:px-4  px-16 sm:px-4 px-16">
        <div className="flexStart flex-1 flex-col">
          <div className="flexCenter flex-row sm:mt-4">
            <Image
              src={images.logo02}
              objectFit="contain"
              width={"32"}
